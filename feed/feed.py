...
class Feed:
    ...
    def refresh(self):
        for rss_feed_url in self.rss_feed_urls:
            ...
            feed_contents = rss_url_parser.parse()
            self._write_feed_contents(feed_contents)

    def _write_feed_contents(self, feed_contents):
        feed_content_formatted = "{feed_title}\n{feed_url}\n{feed_summary}".format(
            feed_title=feed_contents.title, feed_url=feed_contents.url, feed_summary=feed_contents.description)
        with open(RSS_FEED_CONTENT_FILENAME, "w") as feed_content_file:
            feed_content_file.write(feed_content_formatted)
